...
foreach x in spread gr_spread interest_elas ln_spread ln_tbill3yr_gr ln_tbill10yr_gr {
	gen `x'_neg = cond(`ind'<0, `ind', 0)
	gen `x'_pos = cond(`ind'>0, `ind', 0)
}
...
scatter [dep_var]_`y' age if age<25
...
foreach y in a b {	
   forval z = 1/4 {
	  cap drop fitted`y'`z' residual`y'`z' exb_residual`y'`z'
       qui xtreg [dep_var]_`y' ${`y'estimate_`z'}, vce(cluster origin) fe
         ...
         gen exb_`y'`z' = exp(fitted`y'`z')
		 gen exb_residual`y'`z' = exp(residual`y'`z')
...
foreach y in a b {	
   forval z = 1/4 {
		 ...
		 merge m:m parm using "form.dta", nogen
		 save "form.dta", replace
	  }
   }
...
local x = 2018-`year'+1
 line pct_a forecast_base_a forecast_new_a age if origin==`yr' & age<=25, xline(`x') ///
     ylabel(#5, labsize(small)) xlabel(#6, labsize(small)) legend(tstyle(smbody) margin(right)) ///
...
*
forval yr = 1992/2018 {
   local xl = 2018-`year'+1
   line pct_b forecast_base_b forecast_new_b age if origin==`year' & age<=25, xline(`x') ///
     ylabel(#5, labsize(small)) xlabel(#6, labsize(small)) legend(tstyle(smbody) margin(right)) ///
...
line pct_a forecast_base_a forecast_new_a age if age<=25 ///
     ylabel(#5, labsize(small)) xlabel(#6, labsize(small)) legend(tstyle(smbody) symxsize(small) margin(right)) ///
